...
def window_vibor_potoka(streams, name_potoka):# Функция - открывает окно выбора потока 
    ...
    btn_Vibor_potoka = Button(vibor_potoka, text='Выбрать', command=lambda: result_and_exit(), bg='#2C3A55', fg='white')
...
def load_streams(vibor_media_type,url_adress, path_save): # Функция при нажатии на кнопку 'Скачать'
    global v
...
tk.Label(win).grid(row=5, column=5) # Пустая строка1
...
tk.Label(win, text='Вставьте URL ...', font=('Verdana', 10), fg='white', bg='#2C3A55')\
...
tk.Button(win, text='Готово', width=44, font=('Verdana', 10), bg='#2C3A55',fg='#ccc', command=clicked_btn_get)\
    .grid(row=3, column=1) # Кнопка 'Готово'
tk.Button(win, text='Очистить', font=('Verdana', 10), bg='#2C3A55',fg='#ccc', command=lambda:url.delete(0, tk.END))\
    .grid(row=3, column=2, padx=4) # Кнопка 'Очистить'
tk.Button(win, text='Вставить', font=('Verdana', 10), bg='#2C3A55',fg='#ccc', command=insert_btn_get)\
    .grid(row=2, column=2, ipadx=1, padx=15) # Кнопка 'Вставить'

media_var = IntVar()
media_var.set(1)
tk.Label(win, text=' Выберите тип медиа:', font=('Verdana', 10), fg='white', bg='#2C3A55')\
...
tk.Label(win, text='Путь сохранения:', font=('Verdana', 10), fg='white', bg='#2C3A55')\
...
tk.Button(win, text='Изменить', width=8, font=('Verdana', 10), bg='#2C3A55', fg='#ccc', command=insert_btn_path)\
    .grid(row=9, column=2, ipadx=1, padx=15, sticky='w') # Кнопка 'Изменить'

download_button = tk.Button(win, text='Скачать', width=44, font=('Verdana', 10), bg='#2C3A55',fg='#ccc',\
